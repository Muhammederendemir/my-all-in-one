...
build_share() {
    ...
    $MVN_EXEC clean package -pl my-all-in-one-share,my-all-in-one-share-docker
}

build_acs() {
    docker-compose -f "$COMPOSE_FILE_PATH" kill my-all-in-one-acs
    yes | docker-compose -f "$COMPOSE_FILE_PATH" rm -f my-all-in-one-acs
    $MVN_EXEC clean package -pl my-all-in-one-integration-tests,my-all-in-one-platform,my-all-in-one-platform-docker
}
...
prepare_test() {
    $MVN_EXEC verify -DskipTests=true -pl my-all-in-one-platform,my-all-in-one-integration-tests,my-all-in-one-platform-docker
}

test() {
    $MVN_EXEC verify -pl my-all-in-one-platform,my-all-in-one-integration-tests
}
